...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  final _countryController = ValueNotifier<Country>(Country.egypt);
  ...
  Future<void> _sendCode() async {
    ...
    setState(() => _isLoading = true);
    final res = await _authApi.sendCode(email);
    if (!mounted) return;
    setState(() => _isLoading = false);

    if (!res.success) {
      final msg = _mapSendCodeError(res.error ?? '', l10n);
      if (msg == l10n.emailAlreadyExists) {
    ...
    _showSuccess(l10n.verificationSent);
  ...
  Future<void> _verifyOtp() async {
    ...
    FocusScope.of(context).unfocus();
    setState(() => _isLoading = true);
    final res = await _authApi.verifyEmail(email, code);
    if (!mounted) return;
    setState(() => _isLoading = false);
    ...
    _showSuccess(l10n.emailConfirmed);
  ...
  Future<void> _createAccount() async {
    ...
    final fullNameError = fullName.isEmpty ? l10n.fullNameRequired : null;
    final phoneError = phone.isEmpty ? l10n.phoneRequired : null;
    ...
    final res = await _authApi.register(
      ...
      phoneNumber: '${_countryController.value.dialCode}$phone',
      password: password,
    ...
    if (!res.success) {
      final msg = _mapRegisterError(res, l10n);
      if (msg == l10n.emailNotConfirmed || msg == l10n.emailAlreadyExists) {
        setState(() => _currentStep = 1);
      }
      _showError(msg);
      return;
    ...
    _showSuccess(l10n.registrationSuccess);
    if (!mounted) return;
    Navigator.of(context).pushReplacement(
      PageRouteBuilder(
        pageBuilder: (_, _, _) => const HomeScreen(),
  ...
  void _showSuccess(String message) {
    MessageService.showSuccess(context: context, message: message);
  }

  String _mapSendCodeError(String error, AppLocalizations l10n) {
    if (error.contains('already registered') ||
        error.contains('already exists')) {
      return l10n.emailAlreadyExists;
    }
    return error.isNotEmpty ? error : l10n.networkError;
  }

  String _mapRegisterError(ApiResponse res, AppLocalizations l10n) {
    if (res.errors.isNotEmpty) {
      final first = res.errors.first;
      if (first.contains('confirmed') || first.contains('not confirmed')) {
        return l10n.emailNotConfirmed;
      }
      if (first.contains('Duplicate email') ||
          first.contains('already registered')) {
        return l10n.emailAlreadyExists;
      }
      return first;
    }
    return res.error ?? l10n.networkError;
  }
  ...
  Widget _buildActiveStep(bool isTablet) {
    switch (_currentStep) {
      ...
      default:
        return _buildDetailsStep(isTablet);
    }
  }
  ...
  Widget _buildDetailsStep(bool isTablet) {
    return Column(
      children: [
        const SizedBox(height: 10),
        _buildStepHeader(
          title: AppLocalizations.of(context).completeDetails,
        ...
        _StepField(
          ...
          onChanged: (_) {
            if (_fullNameError != null) _clearErrors();
          },
          prefixIcon: const Icon(
        ...
        PhoneNumberField(
          ...
          onChanged: (_) {
            if (_phoneError != null) _clearErrors();
          },
        ),
        const SizedBox(height: 18),
        _StepField(
        ...
        _StepField(
          ...
          onChanged: (_) {
            if (_confirmPasswordError != null) _clearErrors();
          },
          inputFormatters: const [LatinOnlyFormatter()],
          prefixIcon: const Icon(
            Icons.lock_outline_rounded,
          ...
          child: _isLoading
              ...
              : AnimatedBuilder(
                  ...
                  builder: (context, _) => _StepButton(
                    text: AppLocalizations.of(context).createAccountSubmit,
...
class _RegisterGradient extends StatelessWidget {
  const _RegisterGradient();
