...
namespace Taboor_Infrastructure.Persistence.Repositories
{
  ...
  public class RefreshTokenRepository : Repository<RefreshToken>, IRefreshTokenRepository
  {
    ...
    public RefreshTokenRepository(ApplicationDbContext context, ILoggerFactory loggerFactory) : base(context, loggerFactory.CreateLogger<Repository<RefreshToken>>())
    {
    ...
    /// <summary>
    /// Retrieves a stored refresh token record by its value alone, regardless of its state.
    /// Used by cookie-based (web) clients where no access token is available to derive the user.
    /// </summary>
    /// <param name="refreshToken">The raw refresh token value to look up.</param>
    /// <returns>The matching token record, or null when not found.</returns>
    /// <exception cref="ArgumentException">Thrown when the refresh token is invalid.</exception>
    public async Task<RefreshToken?> GetRefreshTokenByHashAsync(string refreshToken)
    {
      try
      {
        ...
        _logger.LogInformation("Looking up refresh token by value");
      ...
      {
        _logger.LogWarning(ex, "Invalid argument while looking up refresh token by value");
        throw;
      }
      catch (Exception ex)
      {
        _logger.LogError(ex, "Unexpected error occurred while looking up refresh token by value");
        throw;
      }
    }
